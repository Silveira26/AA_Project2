...
def plot_randomized(metric_folder, metric_name):
    extracted_files = []
    for root, dirs, files in os.walk("..\\data\\CSV\\results"):
        for file in files:
            extracted_files.append(os.path.join(root, file))

    randomized_files = [f for f in extracted_files if f"randomized\\{metric_folder}\\" in f]
    ...
    for k_value in ['12.5', '25.0', '50.0', '75.0']:
        ...
        plt.title(f'Randomized {metric_name} (K={k_value})')
...
def plot_all(metric_folder, metric_name):
    plot_randomized(metric_folder, metric_name)
